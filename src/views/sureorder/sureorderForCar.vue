    ...
    onCarSure(val, id) {
      ...
      for (var i = 0; i < this.goodsType.length; i++) {
        if (this.goodsType[i].id == id) {
          this.goodsType[i] = Object.assign(this.goodsType[i], {
            buytype: val
          });
          if (val == 1) {
      ...
      this.goodsMsg.allMoney =
        Number(this.goodsMsg.prePrice) + Number(this.goodsMsg.deposit);
    },
    getShopById() {
      this.getselectcommoditybyid(this.shopid).then(res => {
        ...
        for (var i = 0; i < this.goodsDetails.length; i++) {
          if (this.goodsDetails[i].type == 1) {
            this.goodsMsg.deposit +=
              Number(this.goodsDetails[i].deposit) *
              Number(this.goodsDetails[i].count);
            this.goodsMsg.prePrice +=
              Number(this.goodsDetails[i].rentPrice) *
              Number(this.goodsDetails[i].count);
          }
          if (this.goodsDetails[i].type == 2) {
            this.goodsMsg.prePrice +=
              Number(this.goodsDetails[i].price) *
              Number(this.goodsDetails[i].count);
          }
