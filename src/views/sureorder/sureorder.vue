...
import PanelForSureOrder from "./components/panelForSureOrder";
import ShowAddr from "./components/showAddr";
import SureCell from "./components/sureCell";
import SureMsg from "./components/sureMsg";
import SureSubmit from "./components/sureSubmit";
...
export default {
  ...
  methods: {
    ...
    onCarSure(val, id) {
      ...
      if (this.goodsDetails.type == 3) {
        if (val == "1") {
          this.priceList.prePrice =
            Number(this.goodsDetails.price) * Number(this.count);
        }
        if (val == "2") {
          this.priceList.prePrice =
            Number(this.goodsDetails.rentPrice) * Number(this.count);
          this.priceList.deposit =
            Number(this.goodsDetails.deposit) * Number(this.count);
        }
      }

      this.priceList.allMoney =
        Number(this.priceList.prePrice) + Number(this.priceList.deposit);
    },
    getShopById() {
      this.getselectcommoditybyid(this.$route.query.id).then(res => {
        ...
        this.priceList.allMoney =
          this.priceList.prePrice + this.priceList.deposit;
      });
    },
    // 计算提交订单得值
    setOrder() {
      if (this.goodsDetails.type == 1) {
        this.priceListMsg = {
          ...
          rentPriceAll:
            Number(this.goodsDetails.rentPrice) * Number(this.count),
          priceAll: 0,
          depositAll: Number(this.goodsDetails.deposit) * Number(this.count),
      ...
      if (this.goodsDetails.type == 2) {
        this.priceListMsg = {
          id: this.goodsDetails.id,
          rentPriceAll: 0,
          priceAll: Number(this.goodsDetails.price) * Number(this.count),
          depositAll: 0,
          addrid: this.buyAddress.id || "",
          count: this.count,
  ...
  computed: {
    ...mapState(["buyAddress", "goodsSureMsg"]),
    showAddress() {
